...
// Shared key used for ALL remote (Funnel) requests.
const REMOTE_API_KEY = process.env.LLM_API_KEY ?? ''
 ...
 * Remote requests use the shared LLM_API_KEY; local requests use the per-model key.
 * customUrl requests also use the remote key (user-supplied endpoint).
 */
function resolveApiKey(name, customUrl) {
  if (customUrl || USE_REMOTE) return REMOTE_API_KEY || 'lm-studio'
  return ENDPOINTS[name]?.localKey ?? 'lm-studio'
}
