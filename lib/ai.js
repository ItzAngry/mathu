...
// ── Matheus (text-only model) ─────────────────────────────────────────
const MATHEUS_BASE = process.env.MATHEW_API_URL ?? 'http://100.105.232.94:1234/v1'
const MATHEUS_MODEL = process.env.MATHEW_MODEL ?? 'qwen/qwen3-4b-2507'
const MATHEUS_KEY = process.env.MATHEW_API_KEY ?? 'lm-studio'
...
export async function callMatheus({ question, userAnswer, correctAnswer, customUrl, mode = 'check' }) {
  const baseURL = ensureV1(customUrl || MATHEUS_BASE)
  const model = MATHEUS_MODEL
  const apiKey = MATHEUS_KEY
...
export async function callVision({ question, userAnswer, correctAnswer, imageBase64, customUrl }) {
  ...
  // Text answer has highest priority — placed first and clearly labelled
  const userContent = [
    {
      type: 'text',
      text: [
